...
def predicted_proportions(c,mu_r,mu_f,d_r,d_f,tc_bound,r_bound,f_bound,z0):
    ...
    #    TotR = 0;
    #    TotK = 0;
    #    for k in range(int(t_C[0])):
    #        TotR = TotR+p_remember[k];
    #        TotK = TotK+p_know[k];
    #        if(TotK>=know[0]*q/100.0):
    #            if(quant_know[0][j]<=0):
    #                quant_know[0][j]=t[k];
    #                #print "know", sum(p_know[0:k])/know[0];
    #
    #        if(TotR>=rem[0]*q/100.0):
    #            if(quant_rem[0][j]<=0):
    #                quant_rem[0][j]=t[k];
    #                #print "rem", sum(p_remember[0:k])/rem[0];
    #        if(TotK>=know[0]*q/100.0)&(TotR>=rem[0]*q/100.0):
    #            break;
    #
    #            
    #for i in range(1,n):
    ...
    #        TotK = 0;
    #        TotR = 0;
    #        for k in range(int(t_C[i-1])+1,int(t_C[i])):
    #            TotK = TotK+p_know[k];
    #            TotR = TotR+p_remember[k];
    #            if(TotK>=know[i]*q/100.0):
    #                if(quant_know[i][j]<=0):
    #                    quant_know[i][j]=t[k];
    #                    #print "know",sum(p_know[t_C[i-1]+1:k])/know[i];
    #            if(TotR>=rem[i]*q/100.0):
    #                if(quant_rem[i][j]<=0):
    #                    quant_rem[i][j]=t[k];
    #                    #print "rem",sum(p_remember[t_C[i-1]+1:k])/rem[i];
    #            if(TotK>know[i]*q/100.0)&(TotR>rem[i]*q/100.0):
    #                break;
    #                
    ...
    #    TotR = 0;
    #    TotK = 0;
    #    for k in range(int(t_C[n-1])+1,size(t)):
    #            TotK = TotK+p_know[k];
    #            TotR = TotR+p_remember[k];
    #            if(TotK>=know[n]*q/100.0):
    #                if(quant_know[n][j]<=0):
    #                    quant_know[n][j]=t[k];
    #                    #print "know", sum(p_know[t_C[n-1]+1:k])/know[n];
    #            if(TotR>=rem[n]*q/100.0):
    #                if(quant_rem[n][j]<=0):
    #                    quant_rem[n][j]=t[k];
    #                    #print "rem", sum(p_remember[t_C[n-1]+1:k])/rem[n];    
...
#     [[ 0.2251396   0.12817812  0.        ]
#      [ 0.08251064  0.09643134  0.        ]
#      [ 0.01551921  0.02675996  0.        ]
#      [ 0.          0.          0.41311953]]
#     [[ 1.15   1.6    2.15 ]
#      [ 3.2    3.65   4.225]
#      [ 5.5    5.925  6.625]
...
def compute_proportion(parameters): 
    ...
    #c = array([0.7831307,0.60253825]); #parameters[0:n];
    #mu_r = parameters[0]; 
    #mu_f = parameters[1]; 
    #d_r = parameters[2]; 
    #d_f = parameters[3]; 
    #tc_bound = 0.10826691; #parameters[n+4]; 
    #r_bound = 0.49595259; #parameters[n+5];
    #f_bound = 1.34030801; #parameters[n+6];
    #z0 = parameters[4]; #parameters[n+7]; 
    #mu_r0 = -0.14614531; #parameters[n+8]; 
    #mu_f0 = -0.0123076; #parameters[n+9]; 
    #d_r0 = 0.07349555; #parameters[n+10]; 
    #d_f0 = 0.06135621; #parameters[n+11]; 
...
def chi_square_quantile(quantile_edge,remember,know,rem_prop,know_prop,total):
    ...
    for i in range(n+1):
        ...
        for j in range(1,size(QUANT)):
            ...
            total_predicted_rem = total_predicted_rem + predicted_rem;
            total_predicted_know = total_predicted_know +predicted_know;
            total_observed_rem = total_observed_rem + observed_rem;
            total_observed_know = total_observed_know + observed_know
            chi[i] = chi[i]+ ((predicted_rem-observed_rem)**2)/predicted_rem + ((predicted_know-observed_know)**2)/predicted_know;
        chi[i] = chi[i]+(((total*rem_prop[i]-total_predicted_rem)-(size(rem_data)-total_observed_rem))**2)/(total*rem_prop[i]-total_predicted_rem)+(((total*know_prop[i]-total_predicted_know)-(size(know_data)-total_observed_know))**2)/(total*know_prop[i]-total_predicted_know);
    return sum(chi);            
...
# Out[9]:
    ...
    <ipython-input-9-09dae6e01eda> in <module>()
    ----> 1 output = optimize.brute(chi_square,((0.01,1.0),(0.01,1.0),(0.01,1.0),(0.01,1.0),(-0.5,0.5),));
          2 print output;
          3 print chi_square(output);
    

    C:\Users\s sinha\Anaconda\lib\site-packages\scipy\optimize\optimize.pyc in brute(func, ranges, args, Ns, full_output, finish, disp)
       2540     if (N == 1):
       2541         grid = (grid,)
    -> 2542     Jout = vecfunc(*grid)
       2543     Nshape = shape(Jout)
       2544     indx = argmin(Jout.ravel(), axis=-1)
    

    C:\Users\s sinha\Anaconda\lib\site-packages\numpy\lib\function_base.pyc in __call__(self, *args, **kwargs)
    ...
    C:\Users\s sinha\Anaconda\lib\site-packages\numpy\lib\function_base.pyc in _vectorize_call(self, func, args)
       1637                       for _a in args]
       1638 
    -> 1639             outputs = ufunc(*inputs)
       1640 
       1641             if ufunc.nout == 1:
    

    C:\Users\s sinha\Anaconda\lib\site-packages\scipy\optimize\optimize.pyc in _scalarfunc(*params)
       2534     def _scalarfunc(*params):
       2535         params = squeeze(asarray(params))
    -> 2536         return func(params, *args)
       2537 
       2538     vecfunc = vectorize(_scalarfunc)
    

    <ipython-input-8-179a717ac8ef> in chi_square(parm, observed, remember_hit, know_hit, remember_fa, know_fa)
          2     old = sum(observed[0:n+2,0:3]);
          3     new = sum(observed[n+2:2*(n+2),0:3]);
    ----> 4     predicted,quantile_edge_old,quantile_edge_new = compute_proportion(parm);
          5     chi_hit = chi_square_quantile(quantile_edge_old,remember_hit,know_hit,predicted[0:n+1,0],predicted[0:n+1,1],old);
          6     chi_fa = chi_square_quantile(quantile_edge_new,remember_fa,know_fa,predicted[n+2:2*(n+2)-1,0],predicted[n+2:2*(n+2)-1,1],new);
    

    <ipython-input-6-e82e4d132a0e> in compute_proportion(parameters)
         28     d_f0 = 0.06135621; #parameters[n+11];
         29 
    ---> 30     data_old,quant_old = predicted_proportions(c,mu_r,mu_f,d_r,d_f,tc_bound,r_bound,f_bound,z0);
         31     data_new,quant_new = predicted_proportions(c,mu_r0,mu_f0,d_r0,d_f0,tc_bound,r_bound,f_bound,z0);
         32     predicted_data = vstack((data_old,data_new));
    

    <ipython-input-4-db7ef90314c5> in predicted_proportions(c, mu_r, mu_f, d_r, d_f, tc_bound, r_bound, f_bound, z0)
         59         # convolve the particle distribution from the previous timestep
         60         # with the diffusion kernel (using Fourier domain convolution)
    ---> 61         tx[i] = abs(ifftshift(ifft(fft(tx[i-1])*ft_kernel)));
         62         p_pos = tx[i][x>=bound[i]]; # probability of each particle position above the upper bound
         63         x_pos = x[x>=bound[i]];     # location of each particle position above the upper bound
    

    C:\Users\s sinha\Anaconda\lib\site-packages\numpy\fft\fftpack.pyc in fft(a, n, axis)
        164     """
        165 
    --> 166     return _raw_fft(a, n, axis, fftpack.cffti, fftpack.cfftf, _fft_cache)
        167 
        168 
    

    C:\Users\s sinha\Anaconda\lib\site-packages\numpy\fft\fftpack.pyc in _raw_fft(a, n, axis, init_function, work_function, fft_cache)
         75     if axis != -1:
         76         a = swapaxes(a, axis, -1)
    ---> 77     r = work_function(a, wsave)
         78     if axis != -1:
         79         r = swapaxes(r, axis, -1)
